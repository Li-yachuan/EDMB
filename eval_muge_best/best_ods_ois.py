...
record_txt = open(
    "/data/zhoucaixia/workspace/UD_Edge/tmp/trainval_sigma_logit_unetpp_alpha_ffthalf_feat_testalpha_clipsum/alpha_style_all_epoch19/record.txt",
    'w')
...
for eval_name in eval_list:
    ...
    model_R = np.array(model_R).reshape(len(model_name), 100)
    model_p = np.array(model_P).reshape(len(model_name), 100)
    model_F = np.array(model_F).reshape(len(model_name), 100)
    ...
    model_cntR, model_sumR, model_cntP, model_sumP =\
        np.array(model_cntR).reshape(len(model_name), 100), \
        np.array(model_sumR).reshape(len(model_name), 100), \
        np.array(model_cntP).reshape(len(model_name), 100), \
        np.array(model_sumP).reshape(len(model_name), 100)
    ...
    for i_th in range(100):
        ...
        if i_th == 0:
            new_file.writelines(
                "\t\t" + str(int(th[i_th])) + '      ' + str(int(img_cntR[i_th].item())) + "      " + str(
                    int(img_sumR[i_th].item())) + "      " + str(int(img_cntP[i_th].item())) + "      " + str(
                    int(img_sumP[i_th].item())) + "\n")
        else:
            new_file.writelines("\t\t" + str(th_c)[:4] + '      ' + str(int(img_cntR[i_th].item())) + "      " + str(
                int(img_sumR[i_th].item())) + "      " + str(int(img_cntP[i_th].item())) + "      " + str(
                int(img_sumP[i_th].item())) + "\n")
    new_file.flush()
    new_file.close()
